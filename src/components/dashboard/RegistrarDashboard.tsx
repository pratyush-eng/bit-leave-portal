...
export const RegistrarDashboard: React.FC<RegistrarDashboardProps> = ({ onSelectLeaveRequest }) => {
  ...
  const pendingRequests = leaveRequests.filter(r => {
    if (r.status !== 'PENDING_REGISTRAR') return false;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filter Bar */}
      <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-xs flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <ShieldCheck className="w-5 h-5 text-[#3F51B5]" />
          <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500">
            Pending Sanction Queue
          </h3>
        </div>
